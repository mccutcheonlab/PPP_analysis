...
def trialsMultShadedFig(ax, trials, pps = 1, scale = 5, preTrial = 10,
    ...
    if type(trials[0]) != np.ndarray:
        print('Conerting to array')
        trials_array = []
        try:
            trials_array.append(np.array(trials[0]))
            trials_array.append(np.array(trials[1]))
        except:
            print('Could not convert trials to array')
        trials = trials_array
